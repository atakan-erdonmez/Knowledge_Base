...
# S3 Bucket Keys
When using KMS, instead of encrypting individual objects by making API request to KMS, you can locally create unique keys from the Bucket Key. It will have drastic cost savings.
# S3 Object Lock
...
# S3 Access Points
It is an entry point to **prefixes** in your bucket. You can create different access points for finance and sales for example. It simplifies security management.
- It has its own DNS name (Internet Origin or VPC Origin)
- You can enable access of specific [[VPC]]s
You can even use Multi-Region Access Points to have a AWS network accelerated S3 inter-region bucket.
### VPC Origin
